...
module.exports = {
  ...
  log: function log(since, until, callback) {
  ...
  tags: function tags(callback){
    exec('git tag -l v*.*.*', { cwd: cwd }, function(error, stdout, stderr) {
      error = error || stderr;
      if (error) 
  ...
  clone: function clone(url, dir, callback) {
    var cmd = util.format('git clone %s %s', url, dir);
    return exec(cmd, { cwd: cwd }, function(error, stdout, stderr){
      error = error || stderr;
